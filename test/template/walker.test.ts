...
describe("v-if conditional chains", () => {
  ...
  test("v-if / v-else-if / v-else", () => {
    const result = toJsx('<div v-if="a">A</div><div v-else-if="b">B</div><div v-else>C</div>');
...
describe("v-for loops", () => {
  test("basic v-for", () => {
    ...
    expect(result).toBe("{_renderList(items, (item) => (<div key={item.id}>{item.name}</div>))}");
  });

  test("v-for with index", () => {
    const result = toJsx('<div v-for="(item, index) in items" :key="index">{{ item }}</div>');
    expect(result).toBe("{_renderList(items, (item, index) => (<div key={index}>{item}</div>))}");
  });
});
