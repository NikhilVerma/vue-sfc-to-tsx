...
describe('generateAttributes', () => {
  describe('static attributes', () => {
    ...
    test('static class without classMap', () => {
      expect(attrsFor('<div class="foo bar"></div>')).toBe(' class="foo bar"');
    });

    test('static class with classMap', () => {
      const classMap = new Map([['foo', 'styles.foo']]);
      expect(attrsFor('<div class="foo"></div>', classMap)).toBe(' class={styles.foo}');
    });

    test('static class with classMap multiple classes', () => {
      const classMap = new Map([
        ['foo', 'styles.foo'],
        ['bar', 'styles.bar'],
      ]);
      expect(attrsFor('<div class="foo bar"></div>', classMap)).toBe(
        ' class={`${styles.foo} ${styles.bar}`}',
      );
    });

    test('static class mixed mapped and unmapped', () => {
      const classMap = new Map([['active', 'styles.active']]);
      expect(attrsFor('<div class="active plain"></div>', classMap)).toBe(
        ' class={`${styles.active} plain`}',
      );
    });
  ...
  describe('v-bind directives', () => {
    ...
    test(':class with object literal and classMap', () => {
      const classMap = new Map([['active', 'styles.active']]);
      expect(attrsFor(`<div :class="{ active: isActive }"></div>`, classMap)).toBe(
        ' class={{[styles.active]: isActive}}',
      );
    });

    test(':class without classMap passes through', () => {
  ...
  describe('v-on directives', () => {
    ...
    test('@custom-event="handler"', () => {
      expect(attrsFor('<div @custom-event="handler"></div>')).toBe(
